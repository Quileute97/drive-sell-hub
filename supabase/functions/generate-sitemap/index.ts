...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: products, error: prodError } = await supabase
      .from('products')
      .select('slug, updated_at, rating_average, rating_count, view_count')
      .eq('status', 'active')
...
`
  ...
  <!-- Product Pages -->`
      for (const product of products) {
        ...
        let priority = 0.7
        if (product.view_count > 100 || product.rating_count > 10) {
          priority = 0.9
        } else if (product.view_count > 50 || product.rating_count > 5) {
          priority = 0.8
        }
